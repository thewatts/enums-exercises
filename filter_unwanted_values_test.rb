...
class FilterUnwantedValuesTest < Minitest::Test
  ...
  def test_remove_numbers_divisible_by_3
    ...
    remaining = []
    numbers.each { |n| remaining << n if n % 3 != 0 }
  ...
  def test_remove_words_with_more_than_three_letters
    ...
    remaining = []
    words.each { |word| remaining << word if word.length <= 3 }
  ...
  def test_remove_words_ending_in_e
    ...
    remaining = []
    words.each { |w| remaining << w unless w.end_with? "e" }
  ...
  def test_remove_words_ending_in_ing
    ...
    remaining = []
    words.each { |w| remaining << w unless w.end_with? "ing" }
